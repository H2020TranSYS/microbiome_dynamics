...
## run after "MNDA_ISNs_with_Repeat_Faster.R"
...
for (iii in 1:Rep_dist){
  ...
  Qval = rbind(Qval, order(p.adjust(FeaturePval, method = "BH")))
   # }
  ...
  for (nFeat in seq(2,40,1)){
    ...
    for (rep in 1:Rep){
      ...
      for (i in 1:N){
        ...
        FeatureCount[featureSet] = FeatureCount[featureSet] + 1
...
plot(sort(FeatureCount), pch = 20, cex = .5)
order(FeatureCount)

saveRDS(list(AUC,Qval,FeatureCount), file = "Data/Classification_diet_MAGMA_Rep50_D10_L2r0001.rds")
