...
result_path = "C:/Users/fmelo/Documents/GitHub/microbiome_dynamics/Data/MAGMA_data/9M"
graph_path =  "C:/Users/fmelo/Documents/GitHub/microbiome_dynamics/Graphs/MAGMA_graphs/9M"
...
otu_table9M = otu_table[rownames(otu_table) %in% LuckiMap_9M$Sample.name,]
sum(rownames(otu_table9M) %in% LuckiMap_9M$Sample.name)
# 74 ; 69 shared

## SHARED SAMPLES 
otu_table9M_69 = otu_table[rownames(otu_table) %in% Children$Sample.name.y,]
sum(rownames(otu_table9M) %in% Children$Sample.name.y)


write.table(otu_table9M, file = "OTU_TABLE_MAGMA9M.tsv",quote = F,sep = "\t",row.names = T, col.names = T)
write.table(otu_table9M_69, file = "OTU_TABLE_MAGMA9M_69.tsv",quote = F,sep = "\t",row.names = T, col.names = T)

magma_Stool_NoCov <- magma(data = otu_table9M)
plot(magma_Stool_NoCov)

png("graphs_Penders_No_Covariates_MAGMA9M.png", width = 465, height = 225, units='mm', res = 300)
plot(magma_Stool_NoCov)
dev.off()
...
checkStrict(calculating_LOO)

dir.create(file.path(result_path, "LooNet"), showWarnings = FALSE)
dir.create(file.path(result_path, "LooNet_69"), showWarnings = FALSE)


lists = calculating_LOO(otu_table9M, dirs = "LooNet", name = "")
list_not_eliminate_69 = calculating_LOO(otu_table9M_69, dirs = "LooNet_69", name = "_69")
...
otu_table = as.matrix(read.table("OTU_TABLE_MAGMA9M.tsv"))
...
otu_table = as.matrix(read.table("OTU_TABLE_MAGMA9M_69.tsv"))
...
plot_circleplot = function(otu_table, name,Taxonomy_ASVs = Taxonomy_ASVs){
  ...
  png(paste0("9M_MAGMA_", name, "_5thlevel_FAMILY_circle_plot_order.png"), width = 465, height = 225, units='mm', res = 300)
  # magma_Stool_AllCov$
  ...
  png(paste0("9M_MAGMA_", name, "_3rdlevel_CLASS_circle_plot_order.png"), width = 465, height = 225, units='mm', res = 300)
  # magma_Stool_AllCov$
  ...
  png(paste0("9M_MAGMA_", name, "_2ndlevel_PHYLUM_circle_plot_order.png"), width = 465, height = 225, units='mm', res = 300)
  # magma_Stool_AllCov$
  ...
  png(paste0("9M_MAGMA_", name, "_4thlevel_ORDER_circle_plot_order.png"), width = 465, height = 225, units='mm', res = 300)
  # magma_Stool_AllCov$
...
plot_circleplot(otu_table = otu_table, name ="", Taxonomy_ASVs = Taxonomy_ASVs)
